  ...
  async function submit(e) {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: err.response?.data?.message || err.message || 'สมัครสมาชิกไม่สำเร็จ',
        confirmButtonColor: '#facc15',
        customClass: { popup: 'rounded-xl' }
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#fffbe6] to-white flex items-center justify-center px-2">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md border border-yellow-100">
        <h1 className="text-3xl font-bold mb-6 text-yellow-700 text-center">สมัครสมาชิก</h1>
        {msg && <div className="rounded mb-4 px-3 py-2 bg-red-100 text-red-700 border border-red-200 text-center">{msg}</div>}
        <form onSubmit={submit} className="flex flex-col gap-4">
          <input placeholder="ชื่อ-นามสกุล" value={name} onChange={e=>setName(e.target.value)} className="input input-bordered border-yellow-200 focus:border-yellow-400 placeholder-yellow-300 text-yellow-800 bg-white w-full h-12 rounded-lg" required />
          <input placeholder="เบอร์โทรศัพท์" value={phone} onChange={e=>setPhone(e.target.value)} className="input input-bordered border-yellow-200 focus:border-yellow-400 placeholder-yellow-300 text-yellow-800 bg-white w-full h-12 rounded-lg" required />
          ...
          <button className="btn btn-primary text-lg h-12 rounded-lg mt-2 shadow-none" disabled={loading}>{loading ? 'กำลังสมัคร...' : 'สมัครสมาชิก'}</button>
        </form>
        <div className="mt-6 text-center text-yellow-600">
          มีบัญชีอยู่แล้ว?{' '}
          <Link to="/login" className="underline text-yellow-700 hover:text-yellow-900">เข้าสู่ระบบ</Link>
        </div>
      </div>
    </div>
  );
